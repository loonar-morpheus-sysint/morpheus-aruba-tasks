...
# shellcheck disable=SC1091  # aruba_auth.sh is a local file, existence verified at runtime
...
# Validate VRF name
validate_vrf_name() {
    _log_func_enter "validate_vrf_name"
    local vrf_name="$1"

    if [[ -z "${vrf_name}" ]]; then
        log_error "VRF name cannot be empty"
        _log_func_exit_fail "validate_vrf_name" "1"
        return 1
    fi
    ...
    _log_func_exit_ok "validate_vrf_name"
    return 0
}

# Validate VLAN ID
validate_vlan() {
    _log_func_enter "validate_vlan"
    local vlan="$1"

    if [[ -z "${vlan}" ]]; then
        log_error "VLAN ID cannot be empty"
        _log_func_exit_fail "validate_vlan" "1"
        return 1
    fi
    ...
    _log_func_exit_ok "validate_vlan"
    return 0
}

# Validate route distinguisher format
validate_route_distinguisher() {
    _log_func_enter "validate_route_distinguisher"
    local rd="$1"

    # If empty, it's optional so return success
    if [[ -z "${rd}" ]]; then
        _log_func_exit_ok "validate_route_distinguisher"
        return 0
    fi
    ...
    if ! [[ "${rd}" =~ ^[0-9]+:[0-9]+$ ]]; then
        ...
        _log_func_exit_fail "validate_route_distinguisher" "1"
        return 1
    fi
...
prompt_vrf_parameters() {
    _log_func_enter "prompt_vrf_parameters"
    log_info "Entering interactive mode - please provide VRF details"

    # Prompt for VRF name
    while true; do
        read -r -p "Enter VRF name: " VRF_NAME
    ...
    read -r -p "Enter VRF description: " VRF_DESCRIPTION
    if [[ -z "${VRF_DESCRIPTION}" ]]; then
        log_error "VRF description cannot be empty"
        _log_func_exit_fail "prompt_vrf_parameters" "1"
        exit 1
    fi

    # Prompt for VLAN ID
    while true; do
        read -r -p "Enter VLAN ID: " VRF_VLAN
    ...
    read -r -p "Enter route distinguisher (optional, press Enter to skip): " ROUTE_DISTINGUISHER
    ...
    _log_func_exit_ok "prompt_vrf_parameters"
}
