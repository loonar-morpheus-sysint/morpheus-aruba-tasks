...
run_create_vrf() {
    ...
    local args=()
    [[ -z "${ARUBA_VRF_NAME}" ]] && args+=(--name "${ARUBA_VRF_NAME}")
    [[ -z "${ARUBA_FABRIC}" ]] && args+=(--fabric "${ARUBA_FABRIC}")
    [[ -z "${ARUBA_RD}" ]] && args+=(--rd "${ARUBA_RD}")
    [[ -z "${ARUBA_RT_IMPORT}" ]] && args+=(--rt-import "${ARUBA_RT_IMPORT}")
    [[ -z "${ARUBA_RT_EXPORT}" ]] && args+=(--rt-export "${ARUBA_RT_EXPORT}")
    [[ -z "${ARUBA_AF}" ]] && args+=(--af "${ARUBA_AF}")
    [[ -z "${ARUBA_VNI}" ]] && args+=(--vni "${ARUBA_VNI}")
    [[ -z "${ARUBA_SWITCHES}" ]] && args+=(--switches "${ARUBA_SWITCHES}")
    [[ -z "${ARUBA_DESCRIPTION}" ]] && args+=(--description "${ARUBA_DESCRIPTION}")
    [[ -z "${ARUBA_MAX_SESSIONS_MODE}" ]] && args+=(--max-sessions-mode "${ARUBA_MAX_SESSIONS_MODE}")
    [[ -z "${ARUBA_MAX_CPS_MODE}" ]] && args+=(--max-cps-mode "${ARUBA_MAX_CPS_MODE}")
    [[ -z "${ARUBA_MAX_SESSIONS}" ]] && args+=(--max-sessions "${ARUBA_MAX_SESSIONS}")
    [[ -z "${ARUBA_MAX_CPS}" ]] && args+=(--max-cps "${ARUBA_MAX_CPS}")
