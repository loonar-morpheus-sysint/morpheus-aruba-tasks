...
show_troubleshooting() {
  ...
  echo "   → Execute: ./ai-support/scripts/fix-aider-auth.sh"
  printf "%s\n" "   → Ou: export OPENAI_API_KEY=\$(gh auth token | tr -d '\\n\\r[:space:]')"
  echo ""
  ...
  echo "   → Execute: ./ai-support/scripts/fix-aider-auth.sh (configura automaticamente)"
  ...
  echo ""
  echo "📚 Documentação: ./ai-support/docs/AIDER_SETUP.md | Script de correção: ./ai-support/scripts/fix-aider-auth.sh"
  echo ""
...
main() {
  ...
  if [[ ${errors} -eq 0 ]]; then
    ...
    log_info "Para documentação: cat ./ai-support/docs/AIDER_SETUP.md"
