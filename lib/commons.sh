...
# NOTE:
# Do NOT auto-load a .env file here to keep this library side-effect free.
# Scripts that want to load environment variables should explicitly support
# an --env-file flag and source it themselves before calling functions here.
...
# Backward compatibility alias: some scripts use log_warning
log_warning() {
    log_warn "$1"
}
