...
token_needs_refresh() {
  ...
  local current_time
  current_time=$(date +%s)
  local time_until_expiry=$((token_expiry - current_time))
...
get_auth_token() {
  ...
  local current_time
  current_time=$(date +%s)
  local expiry_time=$((current_time + DEFAULT_TOKEN_DURATION))
...
create_vrf() {
  ...
  # Create VRF via API
  local api_url="${FABRIC_COMPOSER_PROTOCOL}://${FABRIC_COMPOSER_IP}:${FABRIC_COMPOSER_PORT}/api/${API_VERSION}/vrfs"
...
main() {
  ...
  log_success "VRF creation process completed successfully"
