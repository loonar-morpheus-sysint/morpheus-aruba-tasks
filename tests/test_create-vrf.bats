...
@test "create-vrf.sh: Script has main function" {
  ...
  run grep -Eq "^main\(\)|^function main" "${PROJECT_ROOT}/create-vrf.sh"
  [ "$status" -eq 0 ]
}

@test "create-vrf.sh: Script validates Aruba connection" {
  # Check for Aruba host validation
  run grep -Eq "ARUBA_HOST|aruba.*auth" "${PROJECT_ROOT}/create-vrf.sh"
  [ "$status" -eq 0 ]
}
